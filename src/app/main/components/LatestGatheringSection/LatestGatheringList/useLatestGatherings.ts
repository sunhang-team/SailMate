...
export function useLatestGatherings() {
  const [page, setPage] = useState(FIRST_PAGE);
  const { data, ...rest } = useQuery(gatheringQueries.main({ limit: MAX_GATHERING_LIMIT }));

  const perPage = usePerPage();
  const latests = data?.latest ?? [];
  const totalPages = Math.max(1, Math.ceil(latests.length / perPage));

  const safePage = Math.min(page, totalPages);
  const startIndex = (safePage - 1) * perPage;
  const visibleGatherings = latests.slice(startIndex, startIndex + perPage);
